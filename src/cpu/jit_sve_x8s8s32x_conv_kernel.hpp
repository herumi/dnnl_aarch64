...
#ifndef CPU_JIT_SVE_X8S8S32X_CONV_KERNEL_HPP
#define CPU_JIT_SVE_X8S8S32X_CONV_KERNEL_HPP
...
struct _jit_sve_x8s8s32x_fwd_kernel : public jit_generator {
    DECLARE_CPU_JIT_AUX_FUNCTIONS(_jit_sve_x8s8s32x_conv_fwd_ker_t)

    enum { STATE_FIRST_DST_LOAD = 0x1U };

    _jit_sve_x8s8s32x_fwd_kernel(jit_conv_conf_t ajcp,
...
struct jit_sve_x8s8s32x_fwd_kernel {
    ...
    jit_sve_x8s8s32x_fwd_kernel(jit_conv_conf_t ajcp,
        ...
        xmm_kernel_(nullptr) {
            ...
            switch (ch_block) {
                case 16:
                    zmm_kernel_ =
                        new _jit_sve_x8s8s32x_fwd_kernel<Xbyak::Zmm>(
                                ajcp, attr);
                    jit_ker = zmm_kernel_->jit_ker_;
                    return;
                case 8:
                    ymm_kernel_ =
                        new _jit_sve_x8s8s32x_fwd_kernel<Xbyak::Ymm>(
                                ajcp, attr);
                    jit_ker = ymm_kernel_->jit_ker_;
                    return;
                case 4:
                    xmm_kernel_ =
                        new _jit_sve_x8s8s32x_fwd_kernel<Xbyak::Xmm>(
                                ajcp, attr);
    ...
    _jit_sve_x8s8s32x_fwd_kernel<Xbyak::Zmm> *zmm_kernel_;
    _jit_sve_x8s8s32x_fwd_kernel<Xbyak::Ymm> *ymm_kernel_;
    _jit_sve_x8s8s32x_fwd_kernel<Xbyak::Xmm> *xmm_kernel_;
};
